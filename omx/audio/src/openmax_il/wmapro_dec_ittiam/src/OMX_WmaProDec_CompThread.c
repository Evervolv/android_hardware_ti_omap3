...
/*****************************************************************************/
/*                                                                           */
/*  File Name        : OMX_WmaProDec_CompThread.c                            */
/*                                                                           */
/*  Description      :                                                       */
/*                                                                           */
...
/* ================================================================================= * */
/**
* @fn WMAPRODEC_ComponentThread() This is component thread that keeps listening for
...
void* WMAPRODEC_ComponentThread (void* pThreadData)
{
    ...
    WMAPRODEC_COMPONENT_PRIVATE* pComponentPrivate = (WMAPRODEC_COMPONENT_PRIVATE*)pThreadData;
    ...
    while (1)
    {
		...
		{
			OMX_PRCOMM2(pComponentPrivate->dbg, "%d :: IsOutputBufAvail: %ld\n",__LINE__, IsOutputBufAvail );
			IsOutputBufAvail = 0;
        ...
        else if (FD_ISSET (pComponentPrivate->cmdPipe[0], &pComponentPrivate->rfds))
        {
            ...
            nRet = WMAPRODEC_HandleCommand (pComponentPrivate);
		...
		process_again: // To allow playback till eof due to large buffers

			   

			eError = WMAPRODEC_HandleDataBuf_FromApp (pComponentPrivate);
		   ...
		   else if(pComponentPrivate->iEOS == 1) // To allow pb till eof due to pr of long buffers
		   {
			   ALOGD("EOS so process called again");
			   goto process_again;
		   }
		}
    }
