...
OMX_ERRORTYPE OMX_ComponentInit (OMX_HANDLETYPE hComp)
{
    ...
    MP3DEC_BUFFERLIST *pTemp = NULL;
    int i=0;

    OMXDBG_PRINT(stderr, PRINT, 1, 0, "%d :: ITTIAM Entering OMX_ComponentInit\n", __LINE__);

    MP3DEC_OMX_CONF_CHECK_CMD(pHandle,1,1);
    ...
    MP3D_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC] =  pCompPort;

    MP3D_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC] = pCompPort;

    MP3D_OMX_MALLOC(pTemp, MP3DEC_BUFFERLIST);
    ...
    for (i=0; i < MAX_NUM_OF_BUFS_MP3DEC; i++)
    {
    ...
    pComponentPrivate->sPortParam->nPorts = NUM_OF_PORTS_MP3DEC;
    ...
    pComponentPrivate->m_shouldConvertToStereo = OMX_FALSE;
    ...
    MP3DEC_OMX_MALLOC_SIZE(pComponentPrivate->sDeviceString,(100*sizeof(OMX_STRING)),OMX_STRING);
    strcpy((char*)pComponentPrivate->sDeviceString,"/eteedn:i0:o0/codec\0");

    /* initialize role name */
    MP3D_OMX_MALLOC(pComponentPrivate->componentRole,OMX_PARAM_COMPONENTROLETYPE);
    ...
    pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC] = pPortDef_ip;
    pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC] = pPortDef_op;
...
//-- #endif
    ...
    pPortDef_ip->nPortIndex                         = INPUT_PORT_MP3DEC;
    ...
    pPortDef_op->nPortIndex                         = OUTPUT_PORT_MP3DEC;
    ...
    pPortDef_op->nBufferSize                        = MIN_MP3D_OUTPUT_BUFFER_SIZE;
    ...
    MP3D_OMX_MALLOC(pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    MP3D_OMX_MALLOC(pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);

    pComponentPrivate->bPreempted = OMX_FALSE;

    /* Set input port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = INPUT_PORT_MP3DEC;
    pPortFormat->nIndex             = OMX_IndexParamAudioMp3;
    pPortFormat->eEncoding          = OMX_AUDIO_CodingMP3;

    /* Set output port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = OUTPUT_PORT_MP3DEC;
    ...
    mp3_ip->nPortIndex = INPUT_PORT_MP3DEC;


    mp3_ip->nSampleRate = MP3D_SAMPLING_FREQUENCY;
    mp3_ip->nChannels = STEREO_STREAM_STEREO;


    mp3_ip->eChannelMode = OMX_AUDIO_ChannelModeStereo;
    mp3_ip->nAudioBandWidth = 0;

    //mp3_ip->eMP3Profile = OMX_AUDIO_MP3ObjectLC;
    ...
    mp3_op->nPortIndex    = OUTPUT_PORT_MP3DEC;
    mp3_op->nBitPerSample = 16;
    mp3_op->nChannels     = STEREO_STREAM_STEREO;
    mp3_op->nSamplingRate = MP3D_SAMPLING_FREQUENCY;
    mp3_op->bInterleaved  = OMX_TRUE;
    OMX_PRCOMM2(pComponentPrivate->dbg, "INIT Sampling Rate:: %d\n",pComponentPrivate->pcmParams->nSamplingRate);
    OMX_PRCOMM2(pComponentPrivate->dbg, "INIT Cannel count :: %d\n",pComponentPrivate->pcmParams->nChannels);

    pComponentPrivate->bPortDefsAllocated = 1;

//Shashank Noise Fix [
    pComponentPrivate->m_numberOfOutputFramesToDrop = 0;
    pComponentPrivate->m_bIsFirstTime               = OMX_TRUE;
//Shashank Noise Fix ]
 ...
 EXIT:
    if(OMX_ErrorNone != eError)
    {
        ...
        MP3DEC_OMX_FREE(pPortDef_ip);
        MP3DEC_OMX_FREE(pPortDef_op);
        MP3DEC_OMX_FREE(mp3_ip);
        MP3DEC_OMX_FREE(mp3_op);
        MP3DEC_OMX_FREE(pTemp);
    }
                                  ...
                                  OMX_U32 nParam,OMX_PTR pCmdData)
{
    ...
    if(pCompPrivate->curState == OMX_StateInvalid)
    {
       OMX_ERROR4(pCompPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
       MP3DEC_OMX_ERROR_EXIT(eError, OMX_ErrorInvalidState,"OMX_ErrorInvalidState");
    }
#endif
#ifdef __PERF_INSTRUMENTATION__
    ...
    case OMX_CommandFlush:
        ...
        {
            MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    case OMX_CommandPortDisable:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: MP3DEC: Entered switch - Command Port Disable \n",__LINE__);
        break;
    case OMX_CommandPortEnable:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: MP3DEC: Entered switch - Command Port Enable \n",__LINE__);
        break;
    case OMX_CommandMarkBuffer:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: MP3DEC: Entered switch - Command Mark Buffer\n",__LINE__);
        if(nParam > 0) {
            MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    ...
    default:
        OMX_ERROR4(pCompPrivate->dbg, "%d :: MP3DEC: Entered switch - Default\n",__LINE__);
    ...
    if (nRet == -1)
    {
        ...
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"write failed: OMX_ErrorInsufficientResources");
    }
    ...
    if (nRet == -1)
    {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"OMX_ErrorInsufficientResources");
    }
                                   ...
                                   OMX_PTR ComponentParameterStructure)
{
    ...
    MP3DEC_OMX_CONF_CHECK_CMD(hComp,1,1)
        pComponentPrivate = (MP3DEC_COMPONENT_PRIVATE *)(((OMX_COMPONENTTYPE*)hComp)->pComponentPrivate);
    MP3DEC_OMX_CONF_CHECK_CMD(pComponentPrivate, ComponentParameterStructure, 1)
    ...
    if(pComponentPrivate->curState == OMX_StateInvalid) {
        OMX_ERROR4(pComponentPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
#endif
    ...
    OMX_PRDSP2(pComponentPrivate->dbg, "%d :: Entering OMX_IndexParamPortDefinition\n", __LINE__);
        if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
                  pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else {
        ...
        if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->nPortIndex) {
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
                pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->nPortIndex){
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else {
            OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from GetParameter \n",__LINE__);
    ...
    OMX_PRDSP2(pComponentPrivate->dbg, "%d :: Entering OMX_IndexParamAudioMp3\n", __LINE__);
                                   ...
                                   OMX_PTR pCompParam)
{
    ...
    MP3DEC_OMX_CONF_CHECK_CMD(pComponentPrivate, pCompParam, 1)

    if (pComponentPrivate->curState != OMX_StateLoaded) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"OMX_ErrorIncorrectStateOperation");
    }
    ...
    case OMX_IndexParamAudioPortFormat:
        {
            ...
            if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[INPUT_PORT_MP3DEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[OUTPUT_PORT_MP3DEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else {
    ...
    case OMX_IndexParamAudioMp3:
        {
            ...
            if(pCompMp3Param->nPortIndex == 0)
            { /* 0 means Input port */
                memcpy(((MP3DEC_COMPONENT_PRIVATE*)
                        pHandle->pComponentPrivate)->mp3Params, pCompMp3Param, sizeof(OMX_AUDIO_PARAM_MP3TYPE));
    ...
    case OMX_IndexParamPortDefinition:
        {
            ...
            if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
               pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC], pCompParam,sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
                    pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC], pCompParam, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else {
                OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from SetParameter",__LINE__);
                eError = OMX_ErrorBadPortIndex;
                                   ...
                                   OMX_PTR pAppData)
{
    ...
    MP3DEC_OMX_CONF_CHECK_CMD(pComponentPrivate,1,1)

    MP3DEC_OMX_CONF_CHECK_CMD(pCallBacks, pCallBacks->EventHandler, pCallBacks->EmptyBufferDone)
    MP3DEC_OMX_CONF_CHECK_CMD(pCallBacks->FillBufferDone, 1, 1)
                                ...
                                OMX_PTR ComponentConfigStructure)
{
    ...
    {
        OMX_PRDSP2(pComponentPrivate->dbg, "%d :: GetConfig OMX_IndexParamAudioMp3 \n",__LINE__);
        if(((OMX_AUDIO_PARAM_MP3TYPE *)(ComponentConfigStructure))->nPortIndex ==
           pComponentPrivate->mp3Params->nPortIndex)
        {
            memcpy(ComponentConfigStructure, pComponentPrivate->mp3Params, sizeof(OMX_AUDIO_PARAM_MP3TYPE));
        }
        else if(((OMX_AUDIO_PARAM_MP3TYPE *)(ComponentConfigStructure))->nPortIndex ==
                pComponentPrivate->pcmParams->nPortIndex)
        {
                                      ...
                                      OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    int ret=0;
    pPortDef = ((MP3DEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[INPUT_PORT_MP3DEC];
    ...
    if (pBuffer == NULL) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"Bad Size");
    }

    if (pBuffer->nInputPortIndex != INPUT_PORT_MP3DEC) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
    }


    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"OMX_ErrorVersionMismatch");
    }
    ...
    if (ret == -1)
    {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorHardware,"write failed: OMX_ErrorHardware");
    }
...
/**
  ...
  *
  * @retval OMX_NoError              Success, ready to roll
  *         OMX_Error_BadParameter   The INPUT_PORT_MP3DEC parameter pointer is null
  **/
/*-------------------------------------------------------------------*/
                                     ...
                                     OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    pPortDef = ((MP3DEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[OUTPUT_PORT_MP3DEC];
    ...
    if (pBuffer == NULL) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: EmptyThisBuffer: Bad Size\n",__LINE__);
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: Bad Size");
    }

    if (pBuffer->nOutputPortIndex != OUTPUT_PORT_MP3DEC) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: EmptyThisBuffer: BadPortIndex\n",__LINE__);
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"write failed: BadPortIndex");
    }
    ...
    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        ...
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pBuffer->nVersion.nVersion:%lu\n",__LINE__,pBuffer->nVersion.nVersion);
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pComponentPrivate->nVersion:%lu\n",__LINE__,pComponentPrivate->nVersion);
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"write failed: OMX_ErrorVersionMismatch");
    }
    if(pComponentPrivate->curState != OMX_StateExecuting && pComponentPrivate->curState != OMX_StatePause) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
...
static OMX_ERRORTYPE ComponentDeInit(OMX_HANDLETYPE pHandle)
{
    ...
    MP3DEC_OMX_CONF_CHECK_CMD(pComponent,1,1)
...
#ifdef RESOURCE_MANAGER_ENABLED
    eError = RMProxy_NewSendCommand(pHandle, RMProxy_FreeResource, OMX_MP3_Decoder_COMPONENT, 0, 3456, NULL);
    if (eError != OMX_ErrorNone) {
    ...
    MP3DEC_OMX_FREE(pComponentPrivate->sDeviceString);
    ...
    OMX_DBG_CLOSE(dbg);
    MP3DEC_OMX_FREE(pComponentPrivate);
                   ...
                   OMX_IN OMX_U32 nSizeBytes)
{
    ...
    MP3DEC_OMX_CONF_CHECK_CMD(hComponent,1,1);
    ...
    MP3DEC_OMX_CONF_CHECK_CMD(pPortDef, 1, 1);
    if (!pPortDef->bEnabled) {
    ...
    MP3DEC_OMX_MALLOC_SIZE(pBufferHeader->pBuffer,(nSizeBytes + DSP_CACHE_ALIGNMENT),OMX_U8);
    ...
    if (nPortIndex == INPUT_PORT_MP3DEC)
    {
    ...
    } else if (nPortIndex == OUTPUT_PORT_MP3DEC) {
    ...
    if((pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->bPopulated == pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->bEnabled)&&
       (pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->bPopulated == pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->bEnabled) &&
 ...
 EXIT:
    if(OMX_ErrorNone != eError) {
        ...
        MP3DEC_OMX_FREE(pBufferHeader->pBuffer);
        MP3DEC_OMX_FREE(pBufferHeader);
    }
                                ...
                                OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if (inputIndex != -1)
    {
        ...
        MP3DEC_OMX_FREE(pComponentPrivate->pInputBufferList->pBufHdr[inputIndex]);
        pComponentPrivate->pInputBufferList->pBufHdr[inputIndex] = NULL;
        pComponentPrivate->pInputBufferList->numBuffers--;

        if (pComponentPrivate->pInputBufferList->numBuffers <
            pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[INPUT_PORT_MP3DEC]->bEnabled &&
    ...
    else if (outputIndex != -1)
    {
        ...
        MP3DEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]->pOutputPortPrivate);
        MP3DEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]);
        ...
        OMX_PRCOMM2(pComponentPrivate->dbg, "pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->nBufferCountMin = %lu\n",pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->nBufferCountMin);
        if (pComponentPrivate->pOutputBufferList->numBuffers <
            pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[OUTPUT_PORT_MP3DEC]->bEnabled &&
           pComponentPrivate->bLoadedCommandPending == OMX_FALSE &&
           !pComponentPrivate->reconfigOutputPort &&
           (pComponentPrivate->curState == OMX_StateIdle ||
            pComponentPrivate->curState == OMX_StateExecuting ||
...
//
    if(!pPortDef->bEnabled) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,
                              "Port is Disabled: OMX_ErrorIncorrectStateOperation");
    }

    if(pPortDef->bPopulated) {
        MP3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,
    ...
    memset((pBufferHeader), 0x0, sizeof(OMX_BUFFERHEADERTYPE));
    if (nPortIndex == OUTPUT_PORT_MP3DEC) {
        pBufferHeader->nInputPortIndex = -1;
        pBufferHeader->nOutputPortIndex = nPortIndex;
