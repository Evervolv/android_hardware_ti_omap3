...
OMX_ERRORTYPE OMX_ComponentInit (OMX_HANDLETYPE hComp)
{
    ...
    WMADEC_BUFFERLIST *pTemp = NULL;
    int i=0;
/*
    {
        FILE *fp;
        fp = fopen("/sdcard/Ittiam.WMA.Decoder.has.been.created","w");
        fclose(fp);
    }
*/
    ...
    WMADEC_OMX_CONF_CHECK_CMD(pHandle,1,1);

	ALOGD("ITTIAM WMA decoder OMX_ComponentInit");
    ...
    WMAD_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[INPUT_PORT_WMADEC] =  pCompPort;

    WMAD_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC] = pCompPort;

    WMAD_OMX_MALLOC(pTemp, WMADEC_BUFFERLIST);
    ...
    for (i=0; i < MAX_NUM_OF_BUFS_WMADEC; i++)
    {
    ...
    WMAD_OMX_MALLOC(pComponentPrivate->sPortParam, OMX_PORT_PARAM_TYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->sPortParam, OMX_PORT_PARAM_TYPE);
    WMAD_OMX_MALLOC(pComponentPrivate->pPriorityMgmt, OMX_PRIORITYMGMTTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pPriorityMgmt, OMX_PRIORITYMGMTTYPE);
    pComponentPrivate->sPortParam->nPorts = NUM_OF_PORTS_WMADEC;
    pComponentPrivate->sPortParam->nStartPortNumber = 0x0;

    pComponentPrivate->wmaParams = NULL;
    ...
    pComponentPrivate->wmaParams = wma_ip;
    pComponentPrivate->pcmParams = wma_op;
    ...
    WMADEC_OMX_MALLOC_SIZE(pComponentPrivate->sDeviceString,(100*sizeof(OMX_STRING)),OMX_STRING);
    strcpy((char*)pComponentPrivate->sDeviceString,"/eteedn:i0:o0/codec\0");

    /* initialize role name */
    WMAD_OMX_MALLOC(pComponentPrivate->componentRole,OMX_PARAM_COMPONENTROLETYPE);
    ...
    pComponentPrivate->pPortDef[INPUT_PORT_WMADEC] = pPortDef_ip;
    pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC] = pPortDef_op;
...
//-- #endif
    ...
    pPortDef_ip->nPortIndex                         = INPUT_PORT_WMADEC;
    pPortDef_ip->eDir                               = OMX_DirInput;
    pPortDef_ip->nBufferCountActual                 = WMAD_NUM_INPUT_BUFFERS;
    pPortDef_ip->nBufferCountMin                    = WMAD_NUM_INPUT_BUFFERS;
    pPortDef_ip->nBufferSize                        = WMAD_INPUT_BUFFER_SIZE;
    ...
    pPortDef_op->nPortIndex                         = OUTPUT_PORT_WMADEC;
    pPortDef_op->eDir                               = OMX_DirOutput;
    pPortDef_op->nBufferCountMin                    = WMAD_NUM_OUTPUT_BUFFERS;
    pPortDef_op->nBufferCountActual                 = WMAD_NUM_OUTPUT_BUFFERS;
    pPortDef_op->nBufferSize                        = MIN_WMAD_OUTPUT_BUFFER_SIZE;
    ...
    WMAD_OMX_MALLOC(pComponentPrivate->pCompPort[INPUT_PORT_WMADEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    WMAD_OMX_MALLOC(pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[INPUT_PORT_WMADEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);

    pComponentPrivate->bPreempted = OMX_FALSE;

    /* Set input port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[INPUT_PORT_WMADEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = INPUT_PORT_WMADEC;
    pPortFormat->nIndex             = OMX_IndexParamAudioWma;
    pPortFormat->eEncoding          = OMX_AUDIO_CodingWMA;

    /* Set output port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = OUTPUT_PORT_WMADEC;
    pPortFormat->nIndex             = OMX_IndexParamAudioPcm;
    pPortFormat->eEncoding          = OMX_AUDIO_CodingPCM;

    /* WMA format defaults */
    OMX_CONF_INIT_STRUCT(wma_ip, OMX_AUDIO_PARAM_WMATYPE);
    wma_ip->nPortIndex = INPUT_PORT_WMADEC;


    /*wma_ip->nSampleRate = WMAD_SAMPLING_FREQUENCY;
    wma_ip->nChannels = STEREO_STREAM_STEREO;


    wma_ip->eChannelMode = OMX_AUDIO_ChannelModeStereo;
    wma_ip->nAudioBandWidth = 0;

    //wma_ip->eWMAProfile = OMX_AUDIO_WMAObjectLC;
    wma_ip->eFormat = OMX_AUDIO_WMAStreamFormatMP1Layer3;*/


    /* PCM format defaults */
    OMX_CONF_INIT_STRUCT(wma_op, OMX_AUDIO_PARAM_PCMMODETYPE);

    wma_op->eNumData      = OMX_NumericalDataSigned;
    wma_op->ePCMMode      = OMX_AUDIO_PCMModeLinear;
    wma_op->nPortIndex    = OUTPUT_PORT_WMADEC;
    wma_op->nBitPerSample = 16;
    wma_op->nChannels     = STEREO_STREAM_STEREO;
    wma_op->nSamplingRate = WMAD_SAMPLING_FREQUENCY;
    wma_op->bInterleaved  = OMX_TRUE;
...
#endif
   eError = WmaDec_StartCompThread(pHandle);
    if (eError != OMX_ErrorNone) {
 ...
 EXIT:
    if(OMX_ErrorNone != eError)
    {
        ...
        WMADEC_OMX_FREE(pPortDef_ip);
        WMADEC_OMX_FREE(pPortDef_op);
        WMADEC_OMX_FREE(wma_ip);
        WMADEC_OMX_FREE(wma_op);
        WMADEC_OMX_FREE(pTemp);
    }
                                  ...
                                  OMX_U32 nParam,OMX_PTR pCmdData)
{
    ...
    if(pCompPrivate->curState == OMX_StateInvalid)
    {
       OMX_ERROR4(pCompPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
       WMADEC_OMX_ERROR_EXIT(eError, OMX_ErrorInvalidState,"OMX_ErrorInvalidState");
    }
#endif
#ifdef __PERF_INSTRUMENTATION__
    ...
    case OMX_CommandFlush:
        ...
        {
            WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    case OMX_CommandPortDisable:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: WMADEC: Entered switch - Command Port Disable \n",__LINE__);
        break;
    case OMX_CommandPortEnable:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: WMADEC: Entered switch - Command Port Enable \n",__LINE__);
        break;
    case OMX_CommandMarkBuffer:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: WMADEC: Entered switch - Command Mark Buffer\n",__LINE__);
        if(nParam > 0) {
            WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    ...
    default:
        OMX_ERROR4(pCompPrivate->dbg, "%d :: WMADEC: Entered switch - Default\n",__LINE__);
    ...
    if (nRet == -1)
    {
        ...
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"write failed: OMX_ErrorInsufficientResources");
    }
    ...
    if (nRet == -1)
    {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"OMX_ErrorInsufficientResources");
    }
                                   ...
                                   OMX_PTR ComponentParameterStructure)
{
    ...
    WMADEC_OMX_CONF_CHECK_CMD(hComp,1,1)
        pComponentPrivate = (WMADEC_COMPONENT_PRIVATE *)(((OMX_COMPONENTTYPE*)hComp)->pComponentPrivate);
    WMADEC_OMX_CONF_CHECK_CMD(pComponentPrivate, ComponentParameterStructure, 1)
    ...
    if(pComponentPrivate->curState == OMX_StateInvalid) {
        OMX_ERROR4(pComponentPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
#endif
    ...
    OMX_PRDSP2(pComponentPrivate->dbg, "%d :: Entering OMX_IndexParamPortDefinition\n", __LINE__);
        if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[INPUT_PORT_WMADEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
                  pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else {
        ...
        if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->nPortIndex) {
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[INPUT_PORT_WMADEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[INPUT_PORT_WMADEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
                pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->nPortIndex){
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else {
            OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from GetParameter \n",__LINE__);
    ...
    OMX_PRDSP2(pComponentPrivate->dbg, "%d :: Entering OMX_IndexParamAudioWma\n", __LINE__);
        if(((OMX_AUDIO_PARAM_WMATYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->wmaParams->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->wmaParams, sizeof(OMX_AUDIO_PARAM_WMATYPE));
                                   ...
                                   OMX_PTR pCompParam)
{
    ...
    OMX_AUDIO_PARAM_WMATYPE *pCompWmaParam = NULL;
    ...
    WMADEC_OMX_CONF_CHECK_CMD(pComponentPrivate, pCompParam, 1)

    if (pComponentPrivate->curState != OMX_StateLoaded) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"OMX_ErrorIncorrectStateOperation");
    }
    ...
    case OMX_IndexParamAudioPortFormat:
        {
            ...
            if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[INPUT_PORT_WMADEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[INPUT_PORT_WMADEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[OUTPUT_PORT_WMADEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else {
    ...
    case OMX_IndexParamAudioWma:
        {
            pCompWmaParam = (OMX_AUDIO_PARAM_WMATYPE *)pCompParam;
            if(pCompWmaParam->nPortIndex == 0)
            { /* 0 means Input port */
                memcpy(((WMADEC_COMPONENT_PRIVATE*)
                        pHandle->pComponentPrivate)->wmaParams, pCompWmaParam, sizeof(OMX_AUDIO_PARAM_WMATYPE));
    ...
    case OMX_IndexParamPortDefinition:
        {
            ...
            if (pComponentParamPort->nPortIndex == 0) {
                ...
                if (pComponentParamPort->format.audio.eEncoding != OMX_AUDIO_CodingWMA) {
            ...
            if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
               pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[INPUT_PORT_WMADEC], pCompParam,sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
                    pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC], pCompParam, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else {
                OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from SetParameter",__LINE__);
                eError = OMX_ErrorBadPortIndex;
                                   ...
                                   OMX_PTR pAppData)
{
    ...
    WMADEC_OMX_CONF_CHECK_CMD(pComponentPrivate,1,1)

    WMADEC_OMX_CONF_CHECK_CMD(pCallBacks, pCallBacks->EventHandler, pCallBacks->EmptyBufferDone)
    WMADEC_OMX_CONF_CHECK_CMD(pCallBacks->FillBufferDone, 1, 1)
                                ...
                                OMX_PTR ComponentConfigStructure)
{
    ...
    if (nConfigIndex ==  OMX_IndexParamAudioWma)
    {
        ...
        if(((OMX_AUDIO_PARAM_WMATYPE *)(ComponentConfigStructure))->nPortIndex ==
           pComponentPrivate->wmaParams->nPortIndex)
        {
            memcpy(ComponentConfigStructure, pComponentPrivate->wmaParams, sizeof(OMX_AUDIO_PARAM_WMATYPE));
        }
                                      ...
                                      OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    int ret=0;
    pPortDef = ((WMADEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[INPUT_PORT_WMADEC];
    ...
    if (pBuffer == NULL) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"Bad Size");
    }

    if (pBuffer->nInputPortIndex != INPUT_PORT_WMADEC) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
    }


    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"OMX_ErrorVersionMismatch");
    }
...
/*    if (pComponentPrivate->bBypassDSP == 0) {
    ...
    if (ret == -1)
    {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorHardware,"write failed: OMX_ErrorHardware");
    }
...
/**
  ...
  *
  * @retval OMX_NoError              Success, ready to roll
  *         OMX_Error_BadParameter   The INPUT_PORT_WMADEC parameter pointer is null
  **/
/*-------------------------------------------------------------------*/
                                     ...
                                     OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    pPortDef = ((WMADEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[OUTPUT_PORT_WMADEC];
    ...
    if (pBuffer == NULL) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: EmptyThisBuffer: Bad Size\n",__LINE__);
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: Bad Size");
    }

    if (pBuffer->nOutputPortIndex != OUTPUT_PORT_WMADEC) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: EmptyThisBuffer: BadPortIndex\n",__LINE__);
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"write failed: BadPortIndex");
    }
    ...
    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        ...
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pBuffer->nVersion.nVersion:%lu\n",__LINE__,pBuffer->nVersion.nVersion);
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pComponentPrivate->nVersion:%lu\n",__LINE__,pComponentPrivate->nVersion);
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"write failed: OMX_ErrorVersionMismatch");
    }
    if(pComponentPrivate->curState != OMX_StateExecuting && pComponentPrivate->curState != OMX_StatePause) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
...
//    OMX_ERROR4(pComponentPrivate->dbg, "%d:: Writing output buffer[%d] into pipe \n",__LINE__, pComponentPrivate->nUnhandledFillThisBuffers );
...
static OMX_ERRORTYPE ComponentDeInit(OMX_HANDLETYPE pHandle)
{
    ...
    WMADEC_OMX_CONF_CHECK_CMD(pComponent,1,1)
    pComponentPrivate = (WMADEC_COMPONENT_PRIVATE *)pComponent->pComponentPrivate;
    WMADEC_OMX_CONF_CHECK_CMD(pComponentPrivate,1,1)
    ...
    eError1 = WMADEC_FreeCompResources(pHandle);
    ...
    WMADEC_OMX_FREE(pComponentPrivate->sDeviceString);
    ...
    OMX_DBG_CLOSE(dbg);
    WMADEC_OMX_FREE(pComponentPrivate);
                   ...
                   OMX_IN OMX_U32 nSizeBytes)
{
    ...
    WMADEC_OMX_CONF_CHECK_CMD(hComponent,1,1);
    ...
    pPortDef = ((WMADEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[nPortIndex];

    WMADEC_OMX_CONF_CHECK_CMD(pPortDef, 1, 1);
    if (!pPortDef->bEnabled) {
    ...
    WMADEC_OMX_MALLOC_SIZE(pBufferHeader->pBuffer,(nSizeBytes + DSP_CACHE_ALIGNMENT),OMX_U8);
    OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: Malloced = %p\n",__LINE__,pBufferHeader->pBuffer);
    pBufferHeader->nVersion.nVersion = WMADEC_BUFHEADER_VERSION;
    ...
    if (nPortIndex == INPUT_PORT_WMADEC)
    {
    ...
    } else if (nPortIndex == OUTPUT_PORT_WMADEC) {
    ...
    if((pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->bPopulated == pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->bEnabled)&&
       (pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->bPopulated == pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->bEnabled) &&
 ...
 EXIT:
    if(OMX_ErrorNone != eError) {
        ...
        WMADEC_OMX_FREE(pBufferHeader->pBuffer);
        WMADEC_OMX_FREE(pBufferHeader);
    }
                                ...
                                OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if (inputIndex != -1)
    {
        ...
        WMADEC_OMX_FREE(pComponentPrivate->pInputBufferList->pBufHdr[inputIndex]);
        pComponentPrivate->pInputBufferList->pBufHdr[inputIndex] = NULL;
        pComponentPrivate->pInputBufferList->numBuffers--;

        if (pComponentPrivate->pInputBufferList->numBuffers <
            pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[INPUT_PORT_WMADEC]->bEnabled &&
    ...
    else if (outputIndex != -1)
    {
        ...
        WMADEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]->pOutputPortPrivate);
        WMADEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]);
        ...
        OMX_PRCOMM2(pComponentPrivate->dbg, "pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->nBufferCountMin = %lu\n",pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->nBufferCountMin);
        if (pComponentPrivate->pOutputBufferList->numBuffers <
            pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[OUTPUT_PORT_WMADEC]->bEnabled &&
           pComponentPrivate->bLoadedCommandPending == OMX_FALSE &&
           !pComponentPrivate->reconfigOutputPort &&
           (pComponentPrivate->curState == OMX_StateIdle ||
            pComponentPrivate->curState == OMX_StateExecuting ||
...
//
    if(!pPortDef->bEnabled) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,
                              "Port is Disabled: OMX_ErrorIncorrectStateOperation");
    }

    if(pPortDef->bPopulated) {
        WMADEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,
    ...
    memset((pBufferHeader), 0x0, sizeof(OMX_BUFFERHEADERTYPE));
    if (nPortIndex == OUTPUT_PORT_WMADEC) {
        pBufferHeader->nInputPortIndex = -1;
        pBufferHeader->nOutputPortIndex = nPortIndex;
            ...
            OMX_OUT OMX_INDEXTYPE* pIndexType)
{
    ...
    if (!(strcmp(cParameterName,"OMX.ITTIAM.index.config.wmadecHeaderInfo"))) {
        *pIndexType = OMX_IndexCustomWmaDecHeaderInfoConfig;
        OMXDBG_PRINT(stderr, DSP, 2, 0, "OMX_IndexCustomWmaDecHeaderInfoConfig\n");
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.index.config.wmadecstreamIDinfo"))){
        *pIndexType = OMX_IndexCustomWmaDecStreamInfoConfig;
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.index.config.wmadec.datapath"))) {
        *pIndexType = OMX_IndexCustomWmaDecDataPath;
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.WMA.Decode.Debug"))) {
