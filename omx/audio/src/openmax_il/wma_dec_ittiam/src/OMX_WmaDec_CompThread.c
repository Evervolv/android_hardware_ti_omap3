...
/*****************************************************************************/
/*                                                                           */
/*  File Name        : OMX_WmaDec_CompThread.c                               */
/*                                                                           */
/*  Description      :                                                       */
/*                                                                           */
...
/* ================================================================================= * */
/**
* @fn WMADEC_ComponentThread() This is component thread that keeps listening for
...
void* WMADEC_ComponentThread (void* pThreadData)
{
	...
	OMX_PRCOMM2(pComponentPrivate->dbg, ":: Component Thread running in while loop, status: %d \n", status);
        ...
        else if (FD_ISSET (pComponentPrivate->cmdPipe[0], &pComponentPrivate->rfds))
        {
            ...
            nRet = WMADEC_HandleCommand (pComponentPrivate);
	...
	pComponentPrivate->bNewOutputBufRequired = 2;
        ...
        if ( (IsInputBufAvail || pComponentPrivate->eos) && IsOutputBufAvail && pComponentPrivate->curState == OMX_StateExecuting &&
			(!pComponentPrivate->reconfigOutputPort) )
	{
	   ...
	   eError = WMADEC_HandleDataBuf_FromApp (pComponentPrivate);
