...
#define PCM_DEC_ROLE "audio_decoder.pcm"
...
static OMX_ERRORTYPE SendCommand (OMX_HANDLETYPE hComp, OMX_COMMANDTYPE nCommand,
                                  OMX_S32 nParam, OMX_PTR pCmdData);
...
OMX_ERRORTYPE OMX_ComponentInit (OMX_HANDLETYPE hComp)
{
    ...
    OMX_AUDIO_PARAM_PCMMODETYPE *aac_ip = NULL;
    OMX_AUDIO_PARAM_PCMMODETYPE *pcm_op = NULL;
    PCMDEC_COMPONENT_PRIVATE *pComponentPrivate;
    AUDIODEC_PORT_TYPE *pCompPort = NULL;
    PCMDEC_BUFFERLIST *pTemp = NULL;
    int i=0;

    OMXDBG_PRINT(stderr, PRINT, 1, 0, "%d :: ITTIAM Entering OMX_ComponentInit\n", __LINE__);

    PCMDEC_OMX_CONF_CHECK_CMD(pHandle,1,1);
    ...
    OMX_DBG_INIT(pComponentPrivate->dbg, "OMX_DBG_PCMDEC");

#ifdef __PERF_INSTRUMENTATION__
    pComponentPrivate->pPERF = PERF_Create(PERF_FOURCC('P','C','M','D'),
                                           PERF_ModuleLLMM |
    ...
    PCMD_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC] =  pCompPort;

    PCMD_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC] = pCompPort;

    PCMD_OMX_MALLOC(pTemp, PCMDEC_BUFFERLIST);
    ...
    for (i=0; i < MAX_NUM_OF_BUFS_PCMDEC; i++)
    {
    ...
    pComponentPrivate->iDownmixtoStereo	  = 0;
    pComponentPrivate->interleave_mode    = 1; //interleaved output by default
    ...
    pComponentPrivate->bDownSampleSbr       = 0;
    ...
    PCMDEC_OMX_MALLOC_SIZE(pComponentPrivate->sDeviceString,(100*sizeof(OMX_STRING)),OMX_STRING);
    strcpy((char*)pComponentPrivate->sDeviceString,"/eteedn:i0:o0/codec\0");

    /* initialize role name */
    PCMD_OMX_MALLOC(pComponentPrivate->componentRole,OMX_PARAM_COMPONENTROLETYPE);
    ...
    pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC] = pPortDef_ip;
    pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC] = pPortDef_op;
...
//-- #endif
    ...
    pPortDef_ip->nPortIndex                         = INPUT_PORT_PCMDEC;
    ...
    pPortDef_op->nPortIndex                         = OUTPUT_PORT_PCMDEC;
    ...
    PCMD_OMX_MALLOC(pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    PCMD_OMX_MALLOC(pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);

    pComponentPrivate->bPreempted = OMX_FALSE;

    /* Set input port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = INPUT_PORT_PCMDEC;
    pPortFormat->nIndex             = OMX_IndexParamAudioPcm;
    pPortFormat->eEncoding          = OMX_AUDIO_CodingPCM;

    /* Set output port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = OUTPUT_PORT_PCMDEC;
    pPortFormat->nIndex             = OMX_IndexParamAudioPcm;
    pPortFormat->eEncoding          = OMX_AUDIO_CodingPCM;

    /* input format defaults */
    OMX_CONF_INIT_STRUCT(aac_ip, OMX_AUDIO_PARAM_PCMMODETYPE);
    aac_ip->nPortIndex = INPUT_PORT_PCMDEC;
	aac_ip->nSamplingRate = PCMD_SAMPLING_FREQUENCY;
    aac_ip->bInterleaved  = OMX_TRUE;
    aac_ip->eNumData      = OMX_NumericalDataSigned;
	aac_ip->ePCMMode      = OMX_AUDIO_PCMModeLinear;
	aac_ip->nBitPerSample = 16;
	aac_ip->nChannels     = STEREO_INTERLEAVED_STREAM_PCMDEC;

    /* PCM format defaults */
    OMX_CONF_INIT_STRUCT(pcm_op, OMX_AUDIO_PARAM_PCMMODETYPE);

    pcm_op->eNumData      = OMX_NumericalDataSigned;
    pcm_op->ePCMMode      = OMX_AUDIO_PCMModeLinear;
    pcm_op->nPortIndex    = OUTPUT_PORT_PCMDEC;
    pcm_op->nBitPerSample = 16;
    pcm_op->nChannels     = STEREO_INTERLEAVED_STREAM_PCMDEC;
    pcm_op->nSamplingRate = PCMD_SAMPLING_FREQUENCY;
    pcm_op->bInterleaved  = OMX_TRUE;
...
#ifdef __PERF_INSTRUMENTATION__
    PERF_ThreadCreated(pComponentPrivate->pPERF, pComponentPrivate->ComponentThread,PERF_FOURCC('P','C','M','T'));
#endif
 ...
 EXIT:
    if(OMX_ErrorNone != eError)
    {
        ...
        PCMDEC_OMX_FREE(pPortDef_ip);
        PCMDEC_OMX_FREE(pPortDef_op);
        PCMDEC_OMX_FREE(aac_ip);
        PCMDEC_OMX_FREE(pcm_op);
        PCMDEC_OMX_FREE(pTemp);
    }
                                  ...
                                  OMX_S32 nParam,OMX_PTR pCmdData)
{
    ...
    if(pCompPrivate->curState == OMX_StateInvalid)
    {
       OMX_ERROR4(pCompPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
       PCMDEC_OMX_ERROR_EXIT(eError, OMX_ErrorInvalidState,"OMX_ErrorInvalidState");
    }
#endif
#ifdef __PERF_INSTRUMENTATION__
    ...
    case OMX_CommandFlush:
        ...
        {
            PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    case OMX_CommandPortDisable:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: PCMDEC: Entered switch - Command Port Disable \n",__LINE__);
        break;
    case OMX_CommandPortEnable:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: PCMDEC: Entered switch - Command Port Enable \n",__LINE__);
        break;
    case OMX_CommandMarkBuffer:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: PCMDEC: Entered switch - Command Mark Buffer\n",__LINE__);
        if(nParam > 0) {
            PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    ...
    default:
        OMX_ERROR4(pCompPrivate->dbg, "%d :: PCMDEC: Entered switch - Default\n",__LINE__);
    ...
    if (nRet == -1)
    {
        ...
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"write failed: OMX_ErrorInsufficientResources");
    }
    ...
    if (nRet == -1)
    {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"OMX_ErrorInsufficientResources");
    }
                                   ...
                                   OMX_PTR ComponentParameterStructure)
{
    ...
    PCMDEC_OMX_CONF_CHECK_CMD(hComp,1,1)
        pComponentPrivate = (PCMDEC_COMPONENT_PRIVATE *)(((OMX_COMPONENTTYPE*)hComp)->pComponentPrivate);
    PCMDEC_OMX_CONF_CHECK_CMD(pComponentPrivate, ComponentParameterStructure, 1)
    ...
    if(pComponentPrivate->curState == OMX_StateInvalid) {
        OMX_ERROR4(pComponentPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
#endif
    ...
    case OMX_IndexParamPortDefinition:
        ...
        if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
                  pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else {
        ...
        if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->nPortIndex) {
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
                pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->nPortIndex){
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else {
            OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from GetParameter \n",__LINE__);
                                   ...
                                   OMX_PTR pCompParam)
{
    ...
    PCMDEC_OMX_CONF_CHECK_CMD(pComponentPrivate, pCompParam, 1)

    if (pComponentPrivate->curState != OMX_StateLoaded) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"OMX_ErrorIncorrectStateOperation");
    }
    ...
    case OMX_IndexParamAudioPortFormat:
        {
            ...
            if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[INPUT_PORT_PCMDEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[OUTPUT_PORT_PCMDEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else {
   ...
   case OMX_IndexParamPortDefinition:
        {
            ...
            if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
               pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC], pCompParam,sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
                    pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC], pCompParam, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else {
                OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from SetParameter",__LINE__);
                eError = OMX_ErrorBadPortIndex;
                                ...
                                OMX_PTR ComponentConfigStructure)
{
    ...
    switch (nConfigIndex)
    {
      case  OMX_IndexCustomFramemodeConfig:
          ...
          break;
      case  OMX_IndexCustomPcmDecStreamConfig:
          pComponentPrivate->interleave_mode = *((int *)ComponentConfigStructure);
          break;
      case OMX_IndexCustomPcmDecDownmixtoStereo:
	      pComponentPrivate->iDownmixtoStereo = *((int *)ComponentConfigStructure);
          break;
      default:
                                   ...
                                   OMX_PTR pAppData)
{
    ...
    PCMDEC_OMX_CONF_CHECK_CMD(pComponentPrivate,1,1)

    PCMDEC_OMX_CONF_CHECK_CMD(pCallBacks, pCallBacks->EventHandler, pCallBacks->EmptyBufferDone)
    PCMDEC_OMX_CONF_CHECK_CMD(pCallBacks->FillBufferDone, 1, 1)
                                ...
                                OMX_PTR ComponentConfigStructure)
{
    ...
    if(nConfigIndex == OMX_IndexCustomDebug)
    {
                                      ...
                                      OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    int ret=0;
    pPortDef = ((PCMDEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[INPUT_PORT_PCMDEC];
#ifdef _ERROR_PROPAGATION__
    if (pComponentPrivate->curState == OMX_StateInvalid){
        eError = OMX_ErrorInvalidState;
    ...
    if (pBuffer == NULL) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"Bad Size");
    }

    if (pBuffer->nInputPortIndex != INPUT_PORT_PCMDEC) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
    }


    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"OMX_ErrorVersionMismatch");
    }
...
/**
  ...
  *
  * @retval OMX_NoError              Success, ready to roll
  *         OMX_Error_BadParameter   The INPUT_PORT_PCMDEC parameter pointer is null
  **/
/*-------------------------------------------------------------------*/
                                     ...
                                     OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    pPortDef = ((PCMDEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[OUTPUT_PORT_PCMDEC];
    ...
    if (pBuffer == NULL) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: FillThisBuffer: Bad Size\n",__LINE__);
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: Bad Size");
    }

    if (pBuffer->nOutputPortIndex != OUTPUT_PORT_PCMDEC) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: EmptyThisBuffer: BadPortIndex\n",__LINE__);
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"write failed: BadPortIndex");
    }
    ...
    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        ...
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pBuffer->nVersion.nVersion:%lu\n",__LINE__,pBuffer->nVersion.nVersion);
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pComponentPrivate->nVersion:%lu\n",__LINE__,pComponentPrivate->nVersion);
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"write failed: OMX_ErrorVersionMismatch");
    }
    if(pComponentPrivate->curState != OMX_StateExecuting && pComponentPrivate->curState != OMX_StatePause) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
...
static OMX_ERRORTYPE ComponentDeInit(OMX_HANDLETYPE pHandle)
{
    ...
    PCMDEC_OMX_CONF_CHECK_CMD(pComponent,1,1)
    ...
    PCMDEC_OMX_FREE(pComponentPrivate->sDeviceString);
    ...
    OMX_DBG_CLOSE(dbg);
    PCMDEC_OMX_FREE(pComponentPrivate);
                   ...
                   OMX_IN OMX_U32 nSizeBytes)
{
    ...
    PCMDEC_OMX_CONF_CHECK_CMD(hComponent,1,1);
    ...
    pPortDef = ((PCMDEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[nPortIndex];

    PCMDEC_OMX_CONF_CHECK_CMD(pPortDef, 1, 1);
    if (!pPortDef->bEnabled) {
    ...
    PCMDEC_OMX_MALLOC_SIZE(pBufferHeader->pBuffer,(nSizeBytes + DSP_CACHE_ALIGNMENT),OMX_U8);
    ...
    if (nPortIndex == INPUT_PORT_PCMDEC)
    {
    ...
    } else if (nPortIndex == OUTPUT_PORT_PCMDEC) {
    ...
    if((pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->bPopulated == pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->bEnabled)&&
       (pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->bPopulated == pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->bEnabled) &&
 ...
 EXIT:
    if(OMX_ErrorNone != eError) {
        ...
        PCMDEC_OMX_FREE(pBufferHeader->pBuffer);
        PCMDEC_OMX_FREE(pBufferHeader);
    }
                                ...
                                OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if (inputIndex != -1)
    {
        ...
        PCMDEC_OMX_FREE(pComponentPrivate->pInputBufferList->pBufHdr[inputIndex]);
        pComponentPrivate->pInputBufferList->pBufHdr[inputIndex] = NULL;
        pComponentPrivate->pInputBufferList->numBuffers--;

        if (pComponentPrivate->pInputBufferList->numBuffers <
            pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[INPUT_PORT_PCMDEC]->bEnabled &&
    ...
    else if (outputIndex != -1)
    {
        ...
        PCMDEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]->pOutputPortPrivate);
        PCMDEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]);
        ...
        OMX_PRCOMM2(pComponentPrivate->dbg, "pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->nBufferCountMin = %lu\n",pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->nBufferCountMin);
        if (pComponentPrivate->pOutputBufferList->numBuffers <
            pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[OUTPUT_PORT_PCMDEC]->bEnabled &&
           pComponentPrivate->bLoadedCommandPending == OMX_FALSE &&
           (pComponentPrivate->curState == OMX_StateIdle ||
            pComponentPrivate->curState == OMX_StateExecuting ||
            pComponentPrivate->curState == OMX_StatePause))
...
//
    if(!pPortDef->bEnabled) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,
                              "Port is Disabled: OMX_ErrorIncorrectStateOperation");
    }

    if(pPortDef->bPopulated) {
        PCMDEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,
    ...
    memset((pBufferHeader), 0x0, sizeof(OMX_BUFFERHEADERTYPE));
    if (nPortIndex == OUTPUT_PORT_PCMDEC) {
        pBufferHeader->nInputPortIndex = -1;
        pBufferHeader->nOutputPortIndex = nPortIndex;
            ...
            OMX_OUT OMX_INDEXTYPE* pIndexType)
{
    ...
    if (!(strcmp(cParameterName,"OMX.ITTIAM.index.config.framemode"))) {
        *pIndexType = OMX_IndexCustomFramemodeConfig;

    }else if (!(strcmp(cParameterName,"OMX.ITTIAM.index.config.stream"))) {
        *pIndexType = OMX_IndexCustomPcmDecStreamConfig;

    }
    else if (!(strcmp(cParameterName,"OMX.ITTIAM.index.config.downmix"))) {
	        *pIndexType = OMX_IndexCustomPcmDecDownmixtoStereo;
