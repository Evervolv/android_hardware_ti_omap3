...
// Shash - Jerky Playback patch [
    pComponentPrivate->framemode          = 1; //Shash
// Shash - Jerky Playback patch ]
#else
...
//-- #endif
    ...
    aac_ip->eAACProfile =  OMX_AUDIO_AACObjectHE_PS;
    aac_ip->nFrameLength =  2048;// use HE_PS frame size as default
                                ...
                                OMX_PTR ComponentConfigStructure)
{
    ...
    switch (nConfigIndex)
    {
      case  OMX_IndexCustomFramemodeConfig:
          ...
          break;
      case  OMX_IndexCustomAacDecStreamConfig:
          pComponentPrivate->interleave_mode = *((int *)ComponentConfigStructure);
          break;
      case OMX_IndexCustomAacDecDownmixtoStereo:
                                ...
                                OMX_PTR ComponentConfigStructure)
{
    ...
    if (nConfigIndex ==  OMX_IndexParamAudioAac)
    {
        ...
        if(((OMX_AUDIO_PARAM_AACPROFILETYPE *)(ComponentConfigStructure))->nPortIndex ==
           pComponentPrivate->aacParams->nPortIndex)
        {
            memcpy(ComponentConfigStructure, pComponentPrivate->aacParams, sizeof(OMX_AUDIO_PARAM_AACPROFILETYPE));
        }
            ...
            OMX_OUT OMX_INDEXTYPE* pIndexType)
{
    ...
    if (!(strcmp(cParameterName,"OMX.ITTIAM.index.config.framemode"))) {
        ...
        *pIndexType = OMX_IndexCustomAacDecStreamConfig;

    }
    else if (!(strcmp(cParameterName,"OMX.ITTIAM.index.config.downmix"))) {
	        *pIndexType = OMX_IndexCustomAacDecDownmixtoStereo;
